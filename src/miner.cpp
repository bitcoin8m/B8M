...
bool ProcessBlockFound(const CBlock* pblock, const CChainParams& chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("Bitcoin8mMiner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, chainparams, NULL, pblock, true, NULL))
        return error("Bitcoin8mMiner: ProcessNewBlock, block not accepted");

    return true;
}

void static Bitcoin8mMiner(const CChainParams& chainparams)
{
    LogPrintf("Bitcoin8mMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bitcoin8m-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in Bitcoin8mMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("Running Bitcoin8mMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("Bitcoin8mMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("Bitcoin8mMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateBitcoin8ms(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Bitcoin8mMiner, boost::cref(chainparams)));
}
