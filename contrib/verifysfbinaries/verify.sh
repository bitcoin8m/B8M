...
###   This script attempts to download the signature file SHA256SUMS.asc from bitcoin8m.org
...
WORKINGDIR="/tmp/bitcoin8m"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bitcoin8m.org/bin/"
VERSIONPREFIX="bitcoin8m-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitcoin8m-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitcoin8m Core binary release signing key installed?"
   fi
