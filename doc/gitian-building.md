...
*Setup instructions for a Gitian build of Bitcoin8m using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitcoin8m
...
- [Building Bitcoin8m](#building-bitcoin8m)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoin8m-host/README.md).
...
Clone the git repositories for bitcoin8m and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoin8m/bitcoin8m
```
...
in the bitcoin8m repository under 'Fetch and build inputs' to install sources which require
...
To build Bitcoin8m (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bitcoin8m repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoin8m/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitcoin8m/bitcoin8m
...
For example:
```bash
URL=https://github.com/laanwj/bitcoin8m.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoin8m=${COMMIT} --url bitcoin8m=${URL} ../bitcoin8m/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoin8m=${COMMIT} --url bitcoin8m=${URL} ../bitcoin8m/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoin8m=${COMMIT} --url bitcoin8m=${URL} ../bitcoin8m/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitcoin8m git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitcoin8m/contrib/gitian-descriptors/*|sort|uniq )
...
B8MPATH=/some/root/path/bitcoin8m.git
SIGPATH=/some/root/path/bitcoin8m-detached-sigs.git

./bin/gbuild --url bitcoin8m=${B8MPATH},signature=${SIGPATH} ../bitcoin8m/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoin8m-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoin8m-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitcoin8m-osx-build.assert
```
...
[bitcoin8m/gitian.sigs](https://github.com/bitcoin8m/gitian.sigs/) repository, or if that's not possible create a pull
