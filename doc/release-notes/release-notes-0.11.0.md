Bitcoin8m Core version 0.11.0 is now available from:

  <https://bitcoin8m.org/bin/bitcoin8m-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer B8M/kB fee to be
...
For example, add the following to `bitcoin8m.conf`:
...
related to the blockchain in the bitcoin8m system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Bitcoin8m Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Bitcoin8m Core will 
...
For an example script see [bitcoin8m-submittx](https://github.com/laanwj/bitcoin8m-submittx).
...
- #5334 `cf87536` libbitcoin8mconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new bitcoin8m-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only Bitcoin8mAmountField
- #5753 `0f44672` Add bitcoin8m logo to about screen
...
- #5809 `46bfbe7` Add bitcoin8m-cli man page
...
- #5849 `41259ca` contrib/init/bitcoin8md.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Bitcoin8m-Qt.app to Bitcoin8m-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for bitcoin8m-tx)
